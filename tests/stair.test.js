...
import { attachStairToBoundary, calculateStairDragLayout, calculateStairLayout, deriveStairDragOptions, deriveStairOpeningSnap, deriveStairSideSegments, deriveStairTreads, detachStairFromBoundary, findStairBoundaryConnection, getStairInterfaceEdge, getStairVertexMap, getStairVertices, materializeStairSideJunction, mergeStairBoundaryConnection, removeStairSideJunction, resolveStairHostEdge, setStairSidePosition, setStairWidth, solveStairLayout, synchronizeConnectedStairLevels, synchronizeHostedStairs, updateStairDimensions, updateStairInterfaceEdgeProperties, validateStairPlacement } from '../src/tools/stairs/stair.js';
...
test('dragging outward from a boundary edge derives a live stair definition', () => {
  ...
  assert.equal(options.treadDepth, 11);
  assert.equal(options.totalRun, 55);
  const attached = attachStairToBoundary(boundary, boundary.edges[0].id, options, makeId);
  assert.equal(attached.stair.dimensions.totalRun, 55);
...
test('stair sides snap geometrically to adjacent boundary nodes without sharing their identity', () => {
  const makeId = ids();
  ...
  const points = getStairVertices(attached.boundary, attached.stair);
  assert.notEqual(attached.stair.anchors.openingStartVertexId, boundary.vertices[0].id);
  assert.notEqual(attached.stair.anchors.openingEndVertexId, boundary.vertices[1].id);
  assert.deepEqual({ x: points[0].x, y: points[0].y }, { x: boundary.vertices[0].x, y: boundary.vertices[0].y });
  assert.deepEqual({ x: points[3].x, y: points[3].y }, { x: boundary.vertices[1].x, y: boundary.vertices[1].y });
  assert.equal(attached.boundary.vertices.length, boundary.vertices.length);
...
test('attaching stairs preserves the complete Deck Boundary and gives the stair independent geometry', () => {
  const makeId = ids();
  const boundary = createDeckBoundary([{ x: 0, y: 0 }, { x: 192, y: 0 }, { x: 192, y: 144 }, { x: 0, y: 144 }], { idFactory: makeId });
  ...
  assert.deepEqual(result.boundary.vertices, boundary.vertices);
  assert.deepEqual(result.boundary.edges, boundary.edges);
  assert.equal(result.stair.geometry.ownership, 'stair');
  assert.equal(result.stair.geometry.vertices.length, 4);
...
test('a hosted stair does not create generated edges inside the Deck Boundary', () => {
  const makeId = ids();
  const boundary = createDeckBoundary([{ x: 0, y: 0 }, { x: 192, y: 0 }, { x: 192, y: 144 }, { x: 0, y: 144 }], { idFactory: makeId });
  ...
  assert.deepEqual(result.stair.generatedEdgeIds, []);
});
...
test('exact stair interface width resizes all four stair anchors without replacing their identities', () => {
  ...
  const byId = getStairVertexMap(resized.boundary, resized.stair);
...
test('plan representation draws only interior riser lines and spaces them by tread depth', () => {
  ...
  const top = getStairVertexMap(attached.boundary, attached.stair).get(attached.stair.anchors.openingStartVertexId);
...
test('dragging one stair side changes width without breaking side parallelism and snaps within six inches', () => {
  ...
  assert.notEqual(snapped.stair.anchors.openingStartVertexId, boundary.vertices[0].id);
  const byId = getStairVertexMap(snapped.boundary, snapped.stair);
...
test('a stair side connection remains a reference and does not split the receiving boundary', () => {
  const makeId = ids();
  const host = createDeckBoundary([{ x: 30, y: 0 }, { x: 150, y: 0 }, { x: 150, y: 144 }, { x: 30, y: 144 }], { idFactory: makeId });
  ...
  assert.deepEqual(connected.boundary.vertices, adjacent.vertices);
  assert.deepEqual(connected.boundary.edges, adjacent.edges);
  assert.equal(connected.stair.sideAttachments.start.relationship, 'shared-boundary-reference');
  assert.equal(connected.stair.sideAttachments.start.junction, undefined);
  ...
  assert.equal(disconnected.stair.sideAttachments.start.relationship, 'shared-boundary-reference');
...
test('a stair side snapped to a host node can detach without moving the deck node', () => {
  ...
  const boundaryById = new Map(detached.boundary.vertices.map((vertex) => [vertex.id, vertex]));
  const stairById = getStairVertexMap(detached.boundary, detached.stair);
  assert.equal(boundaryById.get(host.vertices[0].id).x, 0);
  assert.equal(boundaryById.get(host.vertices[0].id).y, 0);
  assert.equal(stairById.get(detached.stair.anchors.openingStartVertexId).x, 12);
  assert.equal(stairById.get(detached.stair.anchors.outerStartVertexId).x, 12);
});
...
test('a hosted stair follows its construction edge while the Deck Boundary remains continuous', () => {
  const makeId = ids();
  const boundary = createDeckBoundary([{ x: 0, y: 0 }, { x: 120, y: 0 }, { x: 120, y: 144 }, { x: 0, y: 144 }], { idFactory: makeId });
  const attached = attachStairToBoundary(boundary, boundary.edges[0].id, { width: 36, startOffset: 24, totalRise: 24, treadDepth: 10 }, makeId);
  const movedBoundary = { ...boundary, vertices: boundary.vertices.map((vertex) => ({ ...vertex, x: vertex.x + 18, y: vertex.y + 12 })) };
  const synchronized = synchronizeHostedStairs({ objects: [movedBoundary, attached.stair] });
  const stair = synchronized.objects.find((object) => object.type === 'stair');
  const points = getStairVertices(movedBoundary, stair);
  assert.equal(points[0].x, 42);
  assert.equal(points[0].y, 12);
  assert.deepEqual(synchronized.objects.find((object) => object.type === 'deck-boundary').edges, movedBoundary.edges);
});

test('a stair side reports shared-boundary and stair-only roles without changing either owner', () => {
  const makeId = ids();
  const boundary = createDeckBoundary([{ x: 0, y: 0 }, { x: 120, y: 0 }, { x: 120, y: 144 }, { x: 0, y: 144 }], { idFactory: makeId });
  ...
  const byId = getStairVertexMap(attached.boundary, attached.stair);
  ...
  const semanticBoundary = createDeckBoundary([top, midpoint, { x: midpoint.x - 12, y: midpoint.y }, { x: top.x - 12, y: top.y }], { idFactory: makeId });
  semanticBoundary.edges[0].id = 'adjacent-boundary';
  const segments = deriveStairSideSegments(attached.boundary, attached.stair, 'start', [attached.boundary, semanticBoundary]);
  assert.deepEqual(segments.map((segment) => segment.role), ['shared-boundary', 'stair-only']);
