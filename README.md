...
Sprint 2 promotes every boundary segment into a typed construction edge with editable geometry and construction properties. **Stairs**, the first attached construction object, now use a tablet-first press-and-drag workflow with live tread geometry and total-rise feedback. CME keeps every riser at or below 7.5 inches, every tread at or below 11 inches, and treats the transition from the last tread to the deck as the final riser. Topology-aware vertex editing supports moving, inserting, removing, splitting, and intelligently merging neighboring corners while retaining construction meaning whenever the resulting boundary remains valid.
