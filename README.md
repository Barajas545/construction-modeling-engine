...
The floating canvas toolbar is now dedicated to Undo, Redo, and high-frequency visibility controls. It provides quick toggles for Railing, CAT construction lines, and Dimensions without activating their modeling tools. CAT construction lines are stored as an independent serializable layer so the future CAT tool can adopt the control without changing the project format.
